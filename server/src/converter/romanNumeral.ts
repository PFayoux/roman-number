...
/**
 * Convert an arab numeral (integer) into a roman notation (string)
 * @param {number} nb the number to convert
 * @return a string with the roman notation of the numeral
 */
export function arabToRoman (nb: number): string {
  ...
  if(nb >= 5000) {
    throw new Error('The number is too big to be writen in roman numerals')
  }

  if (nb === 0) {
    throw new Error('0 is not convertible to Roman numeral')
  }
...
/**
 * Convert a roman numeral (string) into the arab notation (integer)
 * @param {string} nb 
 */
